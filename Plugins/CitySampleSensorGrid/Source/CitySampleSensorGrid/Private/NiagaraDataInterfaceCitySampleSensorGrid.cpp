...
void UNiagaraDataInterfaceCitySampleSensorGrid::GetFunctionsInternal(TArray<FNiagaraFunctionSignature>& OutFunctions) const
{
...

	{
		...
		SigFindNearest.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(),   TEXT("Out_SensorX")),    NDICitySampleSensorGridLocal::OutputSensorXDescription);
		SigFindNearest.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(),   TEXT("Out_SensorY")),    NDICitySampleSensorGridLocal::OutputSensorYDescription);
		SigFindNearest.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(),   TEXT("Out_OwnerIndex")), NDICitySampleSensorGridLocal::OutputOwnerIndexDescription);
...

	{
		...
		SigReadUserData.AddInput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(), TEXT("OwnerIndex")), NDICitySampleSensorGridLocal::OwnerIndexDescription);
...
bool UNiagaraDataInterfaceCitySampleSensorGrid::UpgradeFunctionCall(FNiagaraFunctionSignature& FunctionSignature)
{
	...
	if (FunctionSignature.Name == NDICitySampleSensorGridLocal::FindNearestName)
	{
			...
			|| FunctionSignature.FunctionVersion == NDICitySampleSensorGridLocal::NodeVersion::Nodeversion_Reworked_Output)
		{
			...
			FunctionSignature.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(),   TEXT("Out_SensorX")),    NDICitySampleSensorGridLocal::OutputSensorXDescription);
			FunctionSignature.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(),   TEXT("Out_SensorY")),    NDICitySampleSensorGridLocal::OutputSensorYDescription);
			FunctionSignature.AddOutput(FNiagaraVariable(FNiagaraTypeDefinition::GetIntDef(),   TEXT("Out_OwnerIndex")), NDICitySampleSensorGridLocal::OutputOwnerIndexDescription);
...
void UNiagaraDataInterfaceCitySampleSensorGrid::PushToRenderThreadImpl()
{
	...
	ENQUEUE_RENDER_COMMAND(FUpdateDI)([RT_Proxy, RT_SensorCount = SensorCountPerSide, RT_GlobalSensorRange = FVector2D(GlobalSensorAccuracy, GlobalSensorRange), RT_UserChannelCount = UserChannelCount](FRHICommandListImmediate& RHICmdList)
	{
