	...
	{	
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			// Only ever bother choosing a next lane when we get near the end of our current lane.
			{
				...
				ChooseNextLaneDistanceFromLaneEnd = FMath::Max(MassTrafficSettings->StopSignBrakingTime * AssumedVehicleSpeed, ChooseNextLaneDistanceFromLaneEnd);
			...
			const EDensityToUseForChoosingLane DensityToUseForChoosingLane = RandomStream.FRand() < MassTrafficSettings->DownstreamFlowDensityQueryFraction ?  ChooseLaneByFunctionalDensity /*rare*/ : ChooseLaneByDownstreamFlowDensity /*common*/;
			...
			if (CurrentLane.NextLanes.IsEmpty())
			{
				...
				#if WITH_MASSTRAFFIC_DEBUG
					UE_VLOG_LOCATION(&MassTrafficSubsystem, TEXT("MassTraffic Validation"), Error, TransformFragments[Index].GetTransform().GetLocation() + FVector(0,0,400), 10.0f, FColor::Red, TEXT("Vehicle is on a lane with no NextLane links (a dead end)"));
				#endif
			
				continue;
			}

			// If we only have one next lane or we're an intersection, we can avoid any lane choosing logic entirely.
			...
			const float SpaceTakenByVehicleOnLane = UE::MassTraffic::GetSpaceTakenByVehicleOnLane(AgentRadiusFragment.Radius, RandomFractionFragment.RandomFraction, MassTrafficSettings->MinimumDistanceToNextVehicleRange);

			FZoneGraphTrafficLaneData* BestNextTrafficLaneData = nullptr;
			float BestNextLaneDensity = TNumericLimits<float>::Max();

			// This lane might be have intersection lanes as next lanes, so lets run through just those and asses the lane they are connected to.
			for (FZoneGraphTrafficLaneData* NextLane : CurrentLane.NextLanes)
			{
				...
				if (!NextLane->ConstData.bIsIntersectionLane)
				{
					...
					const float NextLaneDensity = DensityToUseForChoosingLane == ChooseLaneByDownstreamFlowDensity ?  NextLane->GetDownstreamFlowDensity() : NextLane->FunctionalDensity();
				...
				else // This is an intersection lane.
				{
					...
					{
						UE_LOG(LogMassTraffic, Warning, TEXT("%s - Lane %s is an intersection lane, that should have only one next lane, but it has %d."), ANSI_TO_TCHAR(__FUNCTION__), *NextLane->LaneHandle.ToString(), NextLane->NextLanes.Num());
						continue;
					}
					...
					// We want a different lane than this one.
					if (VehicleControlFragment.ChooseNextLanePreference == EMassTrafficChooseNextLanePreference::ChooseDifferentNextLane && VehicleControlFragment.NextLane == NextLane)
					{
						continue;
					}
			...
			// VisLog
				...
				if (OptionalDebugFragments[Index].bVisLog)
				{
					if (VehicleControlFragment.NextLane)
					{
						...
						UE_VLOG_ARROW(&MassTrafficSubsystem, TEXT("MassTraffic NextLane"), Display, TransformFragments[Index].GetTransform().GetLocation() + FVector(0,0,200), UE::MassTraffic::GetLaneMidPoint(VehicleControlFragment.NextLane->LaneHandle.Index, *ZoneGraphStorage) + FVector(0,0,100), FColor::Blue, TEXT("Next Lane"));
					}
					else
					{
						UE_VLOG_LOCATION(&MassTrafficSubsystem, TEXT("MassTraffic NextLane"), Error, TransformFragments[Index].GetTransform().GetLocation() + FVector(0,0,400), 10.0f, FColor::Red, TEXT("Couldn't Choose Next Lane"));
					}
				}
				#endif
