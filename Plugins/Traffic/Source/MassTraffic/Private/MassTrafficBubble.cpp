...
void FTrafficClientBubbleHandler::DebugValidateBubbleOnServer()
{
	...
	for (int32 OuterIdx = 0; OuterIdx < (*Agents).Num(); ++OuterIdx)
	{
		...
		if (AgentFragment.AgentData.LastUpdateTime == World->GetRealTimeSeconds())
		{
			...
			checkf(FMath::Abs(FMath::FindDeltaAngleRadians(Yaw.GetValue(), OuterItem.Agent.GetYaw())) <= (UE::Mass::Replication::YawReplicateTolerance + KINDA_SMALL_NUMBER), TEXT("Agents yaw different to TransformFragment!"));
		}
	}
}
