...
/** Used for spawning. */
MASSTRAFFIC_API void FindNearestVehiclesInLane(const FMassEntityManager& EntityManager, const FZoneGraphTrafficLaneData& TrafficLaneData, float Distance, FMassEntityHandle& OutPreviousVehicle, FMassEntityHandle& OutNextVehicle);

MASSTRAFFIC_API FVector GetLaneBeginPoint(const uint32 LaneIndex, const FZoneGraphStorage& ZoneGraphStorage, const uint32 CountFromBegin = 0, bool* bIsValid = nullptr);
MASSTRAFFIC_API FVector GetLaneEndPoint(const uint32 LaneIndex, const FZoneGraphStorage& ZoneGraphStorage, const uint32 CountFromEnd = 0, bool* bIsValid = nullptr);
...
MASSTRAFFIC_API bool PointIsNearSegment(const FVector& Point, const FVector& SegmentStartPoint, const FVector& SegmentEndPoint, const float MaxDistance);
