...
struct MASSTRAFFIC_API FMassTrafficLight
{
	...
	FMassTrafficLight(const FVector InPosition, const float InZRotation, const int16 InTrafficLightTypeIndex, const EMassTrafficLightStateFlags InTrafficLightState) :
		Position(InPosition), ZRotation(InZRotation), TrafficLightTypeIndex(InTrafficLightTypeIndex), TrafficLightStateFlags(InTrafficLightState)
	{	
	}
...
struct MASSTRAFFIC_API FMassTrafficLightControl
{
	...
	FMassTrafficLightControl() :
		bIsValid(false), bWillAllVehicleLanesCloseInNextPeriodForThisTrafficLight(true), TrafficLightStateFlags(EMassTrafficLightStateFlags::None)
	{ }
...
struct MASSTRAFFIC_API FMassTrafficIntersectionFragment : public FMassFragment
{
	...
	*/
	...
	void ApplyLanesActionToCurrentPeriod(const EMassTrafficPeriodLanesAction VehicleLanesAction, const EMassTrafficPeriodLanesAction PedestrianLanesAction, UMassCrowdSubsystem* MassCrowdSubsystem, const bool bForce);

	void UpdateTrafficLightsForCurrentPeriod();

	void RestartIntersection(UMassCrowdSubsystem* MassCrowdSubsystem);

	FORCEINLINE void AddTimeRemainingToCurrentPeriod() { PeriodTimeRemaining = PeriodTimeRemaining + GetCurrentPeriod().Duration; }
...
struct FMassTrafficNextVehicleFragment : public FMassFragment
{
	...
	FORCEINLINE bool HasNextVehicle() const { return NextVehicle.IsSet(); }
	
	FORCEINLINE const FMassEntityHandle GetNextVehicle() const { return NextVehicle; }
	...
	FORCEINLINE void UnsafeSetNextVehicle(const FMassEntityHandle NewNextVehicleEntity) { NextVehicle = NewNextVehicleEntity; }

	FORCEINLINE void UnsetNextVehicle() { NextVehicle.Reset(); }
	...
	FORCEINLINE void RemoveLaneChangeNextVehicle(const FMassEntityHandle Entity_Current) { NextVehicles_LaneChange.RemoveAll(Entity_Current); }
...
struct MASSTRAFFIC_API FMassTrafficVehicleLightsFragment : public FMassFragment
{
	...
	FMassTrafficVehicleLightsFragment() 
        : bLeftTurnSignalLights(false), bRightTurnSignalLights(false), bBrakeLights(false)
	{ }
...
struct MASSTRAFFIC_API FMassTrafficVehicleLaneChangeFragment : public FMassFragment
{
	...
	void SetLaneChangeCountdownSecondsToBeAtLeast(const UMassTrafficSettings& MassTrafficSettings, const EMassTrafficLaneChangeCountdownSeconds LaneChangeCountdownSecondsType, const FRandomStream& RandomStream);

	FORCEINLINE bool IsTimeToAttemptLaneChange() const
	{
		return !StaggeredSleepCounterForStartNewLaneChanges && !IsLaneChangeInProgress() && LaneChangeCountdownSeconds <= 0.0f && LaneChangeCountdownSeconds != LaneChangeCountdownSeconds_Uninitialized;
	}
	...
	void EndLaneChangeProgression(FMassTrafficVehicleLightsFragment& VehicleLightsFragment_Current, FMassTrafficNextVehicleFragment& NextVehicleFragment_Current, const FMassEntityManager& EntityManager);
	
	FORCEINLINE bool IsLaneChangeInProgress() const { return LaneChangeSide == EMassTrafficLaneChangeSide::IsLaneChangingToTheLeft || LaneChangeSide == EMassTrafficLaneChangeSide::IsLaneChangingToTheRight; }
...
struct MASSTRAFFIC_API FMassTrafficPIDVehicleControlFragment : public FMassFragment
{
	...
	FMassTrafficPIDVehicleControlFragment() : bHandbrake(false)
	{}

	FMassTrafficPIDVehicleControlFragment(float InMaxSteeringAngle) 
        : MaxSteeringAngle(InMaxSteeringAngle), bHandbrake(false)
	{}
