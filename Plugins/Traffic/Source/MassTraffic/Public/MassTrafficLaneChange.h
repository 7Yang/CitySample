...
struct MASSTRAFFIC_API FMassTrafficLaneChangeFitReport
{
	...
	FMassTrafficLaneChangeFitReport() 
        : bIsClearOfVehicleBehind(true), bIsClearOfVehicleAhead(true), bIsClearOfLaneStart(true), bIsClearOfLaneEnd(true)
	{ }
...
bool FindNearbyVehiclesOnLane_RelativeToDistanceAlongLane(const FZoneGraphTrafficLaneData* TrafficLaneData, const float DistanceAlongLane, FMassEntityHandle& OutEntity_Behind, FMassEntityHandle& OutEntity_Ahead, const FMassEntityManager& EntityManager);
...
bool FindNearbyVehiclesOnLane_RelativeToVehicleEntity(const FZoneGraphTrafficLaneData* TrafficLaneData, const FMassEntityHandle Entity_Current, const FMassTrafficNextVehicleFragment& NextVehicleFragment_Current, FMassEntityHandle& OutEntity_Behind, FMassEntityHandle& OutEntity_Ahead, const FMassEntityManager& EntityManager, const UObject* VisLogOwner = nullptr);
...
FZoneGraphLaneLocation GetClosestLocationOnLane(const FVector& Location, const int32 LaneIndex, const float MaxSearchDistance, const FZoneGraphStorage& ZoneGraphStorage, float* OutDistanceSquared = nullptr);
	
void ChooseLaneForLaneChange(const float DistanceAlongCurrentLane_Initial, const FZoneGraphTrafficLaneData* TrafficLaneData_Initial, const FAgentRadiusFragment& AgentRadiusFragment, const FMassTrafficRandomFractionFragment& RandomFractionFragment, const FMassTrafficVehicleControlFragment& VehicleControlFragment, const FRandomStream& RandomStream, const UMassTrafficSettings& MassTrafficSettings, FMassTrafficLaneChangeRecommendation& OutRecommendation);
