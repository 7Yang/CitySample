...
struct MASSTRAFFIC_API FMassTrafficSimpleTrailerConstraintSolver
{
	...
	void ApplyConstraints(const float Dt, const Chaos::FPBDJointSolverSettings& SolverSettings, const Chaos::FPBDJointSettings& JointSettings);

	FORCEINLINE bool GetIsActive() const { return bIsActive; }
	FORCEINLINE const FVector& GetP(const int32 Index) const { return Ps[Index]; }
	FORCEINLINE const FQuat& GetQ(const int32 Index) const { return Qs[Index]; }

private:
	bool UpdateIsActive();
	void UpdateDerivedState();

	void ApplyPositionConstraints(float Dt, const Chaos::FPBDJointSolverSettings& SolverSettings, const Chaos::FPBDJointSettings& JointSettings);
	void ApplyRotationConstraints(float Dt, const Chaos::FPBDJointSolverSettings& SolverSettings, const Chaos::FPBDJointSettings& JointSettings);
	...
	float GetTwistStiffness( const Chaos::FPBDJointSolverSettings& SolverSettings, const Chaos::FPBDJointSettings& JointSettings) 
    {
		return (SolverSettings.TwistStiffnessOverride >= 0.0f) ? SolverSettings.TwistStiffnessOverride : JointSettings.Stiffness;
	}

	float GetSwingStiffness(const Chaos::FPBDJointSolverSettings& SolverSettings, const Chaos::FPBDJointSettings& JointSettings)
	{
		return (SolverSettings.SwingStiffnessOverride >= 0.0f) ? SolverSettings.SwingStiffnessOverride : JointSettings.Stiffness;
	}

	void ApplyRotationConstraintDD(const float JointStiffness, const FVector& Axis, const float Angle);
	static void GetEllipticalConeAxisErrorLocal(const FQuat& R0, const FQuat& R1, const float SwingLimitY, const float SwingLimitZ, FVector& AxisLocal, float& Error);
	static void GetCircularConeAxisErrorLocal(const FQuat& R0, const FQuat& R1, const float SwingLimit, FVector& AxisLocal, float& Error);
...
{
    MASSTRAFFIC_API void ExtractPhysicsVehicleConfig(AWheeledVehiclePawn* PhysicsActor, FMassTrafficSimpleVehiclePhysicsConfig& OutVehicleConfig, FMassTrafficSimpleVehiclePhysicsSim& OutVehicleSim);
}
