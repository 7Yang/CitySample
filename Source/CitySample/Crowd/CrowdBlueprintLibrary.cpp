...
void UCitySampleCrowdFunctionLibrary::SetMeshComponents(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* BaseComponent, USkeletalMeshComponent* BodyComponent, USkeletalMeshComponent* HeadComponent, USkeletalMeshComponent* TopComponent, USkeletalMeshComponent* BottomComponent, USkeletalMeshComponent* ShoesComponent, UStaticMeshComponent* AccessoryComponent)
{	
...
void UCitySampleCrowdFunctionLibrary::SetGroomComponents(const UCrowdCharacterDataAsset* CrowdCharacterDataAsset, const FCrowdCharacterDefinition& CharacterDefinition, UGroomComponent* HairComponent, UGroomComponent* EyebrowsComponent, UGroomComponent* FuzzComponent, UGroomComponent* EyelashesComponent, UGroomComponent* MustacheComponent, UGroomComponent* BeardComponent, bool bUseCards, bool bEnableGroomBinding)
{
	// Create an array where the order matches the hair slots enum order
	TArray<UGroomComponent*> GroomComponents = { HairComponent, EyebrowsComponent, FuzzComponent, EyelashesComponent, MustacheComponent, BeardComponent };
...
void UCitySampleCrowdFunctionLibrary::SetBodyMaterials(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* BodyComponent, const FName MaterialSlotName, const FName BodyParameterName, USkeletalMeshComponent* BottomComponent /*=nullptr*/)
{
	if (!BodyComponent)
	{
		return;
...
void UCitySampleCrowdFunctionLibrary::SetHairMaterials(const FCrowdCharacterDefinition& CharacterDefinition, UGroomComponent* HairComponent, const FName MelaninParameterName, const FName RednessParameterName, const FName RoughnessParameterName)
{
	if (!HairComponent)
	{
		return;
...
void UCitySampleCrowdFunctionLibrary::SetHeadMaterials(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* HeadComponent, const FName MaterialSlotName, const FName FaceParameterName, const FName ChestParameterName, const int LODIndex)
{
	if (!HeadComponent)
	{
		return;
	...
	if (ensure(NumHairSlots <= CharacterDefinition.HairDefinitions.Num()))
	{
		for (const FHairMapInfo& HairMapInfo : HairMaps)
		{
			...
			if ((bShouldUseBakedGroomMap || HairDefinition.Groom.IsNull()) && !HairDefinition.BakedGroomMap.IsNull())
			{
				...
				if (!BakedGroomMap)
				{
					UE_LOG(LogCitySample, Warning, TEXT("HairDefinition.BakedGroomMap ResolveSoftObjectPtr failed. BakedGroomMap asset name: '%s'."), *HairDefinition.BakedGroomMap.GetAssetName());
				}
...
void UCitySampleCrowdFunctionLibrary::SetOutfitMaterials(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* TopComponent, USkeletalMeshComponent* BottomComponent, USkeletalMeshComponent* ShoesComponent, UStaticMeshComponent* AccessoryComponent)
{
...
void UCitySampleCrowdFunctionLibrary::GenerateCharacterDefinitionFromOptions(const FCrowdCharacterOptions& CharacterOptions, const UCrowdCharacterDataAsset* DataAsset, FCrowdCharacterDefinition& CharacterDefinition)
{
	...
	const FName MorphTargetName, const TArray<int32> Sections, const int32 UVChannel)
{
	...
	{	
		...
		if (BaseMesh != SourceMesh)
		{
			...
			if (!GetSkeletalMeshUVMapping(*BaseMesh, LODIndex, UVChannel, *SourceMesh, LODIndex, UVChannel, BaseVertices, SourceVertices, BaseToSourceMapping))
			{
				continue;
			}
		}
			...
			if (!Sections.IsEmpty())
			{
				...
				for (int32 SectionIndex = 0; SectionIndex < Sections.Num(); ++SectionIndex)
				{
					if (BaseMeshLODModel.Sections.IsValidIndex(SectionIndex))
					{
						...
						if (BaseVertexIndex >= (int32)Section.BaseVertexIndex && BaseVertexIndex < (int32)Section.NumVertices)
						{
...
bool UCitySampleCrowdFunctionLibrary::GetSkeletalMeshUVMapping(const USkeletalMesh& SkeletalMesh, const int32 LODIndex, const int32 UVChannel, const USkeletalMesh& OtherSkeletalMesh, const int32 OtherLODIndex, const int32 OtherUVChannel, TArray<FVector>& Vertices, TArray<FVector>& OtherVertices, TArray<int32>& Mapping)
{
	Vertices.Reset();
	OtherVertices.Reset();
	Mapping.Reset();
