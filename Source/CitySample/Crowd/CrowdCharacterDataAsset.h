...
public:
    ...
    */
    ...
    FCrowdCharacterDefinition GetCharacterDefinition(const ECitySampleCrowdGender Skeleton, const ECitySampleCrowdBodyType BodyType, const int32 HeadIndex, const int32 OutfitIndex, const int32 OutfitMaterialIndex, const int32 HairIndex, const int32 HairColorIndex, const int32 AccessoryIndex, const int32 SkinTextureIndex, const int32 SkinTextureModifierIndex, const int32 ScaleFactorIndex) const;
    ...
    UFUNCTION(BlueprintCallable)
    TArray<UGroomAsset*> FindGrooms(const bool bMale = true, const bool bFemale = true, const bool bHair = true, const bool bEyebrows = true, const bool bFuzz = true, const bool bEyelashes = true, const bool bMustache = true, const bool bBeard = true) const;

    /* Finds All Unique Groom Meshes used in all Definitions.
    * If GroupIndex is INDEX_NONE all LOD meshes will be returned.
    */
    UFUNCTION(BlueprintCallable)
    TArray<UStaticMesh*> FindGroomMeshes(const bool bMale = true, const bool bFemale = true, const bool bHair = true, const bool bEyebrows = true, const bool bFuzz = true, const bool bEyelashes = true, const bool bMustache = true, const bool bBeard = true, const int32 GroupIndex = -1 /*INDEX_NONE*/) const;
    ...
    /* Loads all Definitions from a JSON file. All existing definitions will be replaced */
    //UFUNCTION(BlueprintCallable)
    //bool LoadFromJSON(const FString& FilePath);

    /* Save Definitions to a JSON file.  The JSON file with be stored in the Project Saved directory. */
    //UFUNCTION(BlueprintCallable)
    //bool SaveToJSON();
