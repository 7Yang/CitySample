...
struct FCrowdLineupInstance
{
	...
	FCrowdLineupInstance(AActor* InActor, const FIntVector InCoordinates, const FCrowdCharacterOptions InOptions)
		: LineupActor(InActor), LineupCoordinates(InCoordinates), InstanceOptions(InOptions)
	{ }
...
private:
	struct FCrowdVariationSpecifier
	{
		...
		FCrowdVariationSpecifier(const bool bInEnabled, const EAxis::Type InTargetAxis, const ECrowdLineupVariation InVariation, const int InValue = 0)
			: bEnabled(bInEnabled), TargetAxis(InTargetAxis), Variation(InVariation), CurrentValue(InValue)
		{ }
...
public:
	...
	AActor* SpawnLineupActor(const FIntVector SpawnCoordinates, const FString& Label, FVector SpawnLocation, FRotator SpawnRotation, const FCrowdCharacterDefinition CharacterDefinition, const FCrowdCharacterOptions CharacterOptions);
