...
ACrowdCharacterLineup::ACrowdCharacterLineup()
	: Spacing(500, 70, 180), LocationOffset(0, 0, 117), RotationOffset(0, 180, 0)
{
	SceneComponent = CreateDefaultSubobject<USceneComponent>(TEXT("SceneComponent"));
	RootComponent  = SceneComponent;

#if WITH_EDITOR
	if (RootComponent) { RootComponent->bVisualizeComponent = true; }
#endif
}
		...
		for (int FirstIndex = 0; FirstIndex < MaxFirst; ++FirstIndex)
		{
			...
			for (int SecondIndex = 0; SecondIndex < MaxSecond; ++SecondIndex)
			{
				...
				for (int ThirdIndex = 0; ThirdIndex < MaxThird; ++ThirdIndex)
				{
					...
					FIntVector LineupCoordinates = FirstVariation.GetOffsetDirection(FirstIndex) + SecondVariation.GetOffsetDirection(SecondIndex) + ThirdVariation.GetOffsetDirection(ThirdIndex);
...
int ACrowdCharacterLineup::GetNumberOfVariants(const FCrowdCharacterOptions& CharacterOptions, ECrowdLineupVariation VariantType) const
{
	...
	case ECrowdLineupVariation::Head:
	{
		...
		FCrowdBodyOutfitDefinition& BodyOutfitDefinition = CharacterOptions.BodyType == ECitySampleCrowdBodyType::NormalWeight ?  GenderDefinition.NormalWeight : CharacterOptions.BodyType == ECitySampleCrowdBodyType::OverWeight ? GenderDefinition.OverWeight : GenderDefinition.UnderWeight;
...
void ACrowdCharacterLineup::UpdateInstanceTransforms()
{
	for (FCrowdLineupInstance& LineupInstance : LineupInstances)
	{
		...
		if (IsValid(LineupActor))
		{
			...
			FTransform NewTransform(RotationOffset, LocationOffset + (Spacing * FVector(LineupInstance.LineupCoordinates)), CurrentScale);
