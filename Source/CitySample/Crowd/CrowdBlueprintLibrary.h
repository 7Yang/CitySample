...
public:
	...
	*/
	...
	UFUNCTION(BlueprintCallable)
	static void SetGroomComponents(const UCrowdCharacterDataAsset* CrowdCharacterDataAsset, const FCrowdCharacterDefinition& CharacterDefinition, UGroomComponent* Hair, UGroomComponent* Eyebrows, UGroomComponent* Fuzz, UGroomComponent* Eyelashes, UGroomComponent* Mustache, UGroomComponent* Beard, bool bUseCards=true, bool bEnableGroomBinding=true);
	...
	*/
	UFUNCTION(BlueprintCallable)
	static void SetHairMaterials(const FCrowdCharacterDefinition& CharacterDefinition, UGroomComponent* HairComponent, const FName MelaninParameterName, const FName RednessParameterName, const FName RoughnessParameterName);

	/* Set Head Material Instance Parameters
	*  This overrides the texture for face and chest.
	*/
	UFUNCTION(BlueprintCallable)
	static void SetHeadMaterials(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* HeadComponent, const FName MaterialSlotName, const FName FaceParameterName, const FName ChestParameterName, const int LODIndex);

	/* Set Head Material Instance Parameters
	*/
	UFUNCTION(BlueprintCallable)
	static void SetBodyMaterials(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* BodyComponent, const FName MaterialSlotName, const FName BodyParameterName, USkeletalMeshComponent* BottomComponent = nullptr);

	/* Sets Dynamic Material Instances for Outfit.
	*/
	UFUNCTION(BlueprintCallable)
	static void SetOutfitMaterials(const FCrowdCharacterDefinition& CharacterDefinition, USkeletalMeshComponent* TopComponent, USkeletalMeshComponent* BottomComponent, USkeletalMeshComponent* ShoesComponent, UStaticMeshComponent* AccessoryComponent);
	...
	*/
	UFUNCTION(BlueprintCallable)
	static void SetGroomInterpolationType(UGroomAsset* Groom, EGroomInterpolationType InterpolationType, bool bUseRBF=false);
	...
	*/
	UFUNCTION(BlueprintCallable, meta = (AutoCreateRefTerm = "Sections"))
	static bool AddMorphTarget(USkeletalMesh* BaseMesh, USkeletalMesh* SourceMesh, USkeletalMesh* TargetMesh, const FName MorphName, const TArray<int32> Sections, const int32 UVChannel = 0);

	/* Returns SkeletalMesh Vertex Position */
	static int32 GetSkeletalMeshVertices(const USkeletalMesh& Mesh, const int32 LODIndex, TArray<FVector>& Vertices);

	/* Returns SkeletalMesh Vertex Position and UVs */
	static int32 GetSkeletalMeshVerticesAndUVs(const USkeletalMesh& Mesh, const int32 LODIndex, const int32 UVChannel, TArray<FVector>& Vertices, TArray<FVector2D>& UVs);

	/*
	* Computes Vertex Mapping between 2 SkeletalMeshes using UV coordinates */
	static bool GetSkeletalMeshUVMapping(const USkeletalMesh& SkeletalMesh, const int32 LODIndex, const int32 UVChannel, const USkeletalMesh& OtherSkeletalMesh, const int32 OtherLODIndex, const int32 OtherUVChannel, TArray<FVector>& Vertices, TArray<FVector>& OtherVertices, TArray<int32>& Mapping);
...
private:
	...
	FORCEINLINE static void SetGroomComponent(UGroomComponent* GroomComponent, TSoftObjectPtr<UGroomAsset> GroomPtr, UGroomBindingAsset* GroomBinding, bool bUseCards)
	{
		if (GroomComponent)
		{
			// Set Groom
