...
UCitySamplePanel* UCitySamplePanel::AddChildPanel(UCitySamplePanel* ChildPanel, UPanelWidget* ContainerWidget, bool bSkipAnimation)
{
	...
	if (ContainerWidget)
	{
		...
		{
			UE_LOG(LogCitySampleUI, Warning, TEXT("AddChildPanel %s failed. %s must be a sub-widget of %s."), *ChildPanel->GetName(), *ContainerWidget->GetName(), *GetName() );
			return nullptr;
		}

		if (!ContainerWidget->CanAddMoreChildren())
		{
			UE_LOG(LogCitySampleUI, Warning, TEXT("AddChildPanel %s failed. %s cannot add more children."), *ChildPanel->GetName(), *ContainerWidget->GetName() );
			return nullptr;
		}
	}
	...
	if (UCitySamplePanel* PreviousParent = ChildPanel->Parent)
	{
		if (PreviousParent == this)
		{
			...
			// 1. The Widget is already in that container and a new AddChildPanel is called again. In that case, ignore the add and return the widget already there
			// 2. The widget was removed and re-added while the OutAnimation is running so remove is already postponed until the animation stops. Set the PendingState to make sure the ChildPanel we be added when the animation stops. 
