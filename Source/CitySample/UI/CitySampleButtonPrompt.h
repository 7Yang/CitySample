...
public:
	...
	virtual bool HasControlsFlavor() const override { return ControlsFlavorImageConfigMap.Contains(GetControlsFlavor()); }
	
	/** Gets the current button image config struct. */
	UFUNCTION(BlueprintPure, Category = "CitySample Button")
	const FCitySampleImageConfig& GetImageConfig() const { return ButtonImageConfig; }
	...
	UFUNCTION(BlueprintCallable, Category = "CitySample Button")
	void SetIsPressed(const bool bShouldBePressed) { bIsPressed = bShouldBePressed; }
