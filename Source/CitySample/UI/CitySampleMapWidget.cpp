...
void UCitySampleMapWidget::NativeOnInitialized()
{
	...
	ACitySampleWorldInfo* WorldInfo = Cast<ACitySampleWorldInfo>(UGameplayStatics::GetActorOfClass(GetWorld(), ACitySampleWorldInfo::StaticClass()));
	USceneCaptureComponent2D* SceneCapture = WorldInfo ? WorldInfo->GetWorldMapSceneCapture() : nullptr;
...
bool UCitySampleMapWidget::IsMapPositionWithinBounds(const FVector2D& MapPosition)
{
	...
	return (MapPosition.X < MaxPosition.X) && (MapPosition.X > MinPosition.X) && (MapPosition.Y < MaxPosition.Y) && (MapPosition.Y > MinPosition.Y);
}
...
void UCitySampleMapWidget::InitializeMapViewProjectionMatrix(USceneCaptureComponent2D* const SceneCapture2D)
{
	...
	UGameplayStatics::CalculateViewProjectionMatricesFromMinimalView(InMapViewInfo, InCustomProjectionMatrix, MapViewMatrix, MapProjectionMatrix, MapViewProjectionMatrix);
}
