...
void UCitySampleCamera_PhotoMode::UpdateExposureSettings(const struct FPhotoModeSettings& Settings, struct FTViewTarget& OutVT) const
{
...
void UCitySampleCamera_PhotoMode::UpdateFocusSettings(UPhotoModeComponent* PhotoModeComponent, const struct FPhotoModeSettings& Settings, UCineCameraComponent* CineCamComp, const struct FTViewTarget& VT)
{
	...
	if (Settings.bAutoFocusEnabled)
	{
		...
		FTraceHandle ASyncFocusTrace = GetWorld()->AsyncSweepByChannel(EAsyncTraceType::Single, TraceStart, TraceEnd, FQuat::Identity, ECC_WorldStatic, TraceShape, TraceParams, FCollisionResponseParams::DefaultResponseParam, &AsyncFocusTraceDelegate);
