...
void FCitySampleAnimNode_RequestInertialization::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	...
	const bool bSkipForBecomingRelevant = GetSkipOnBecomingRelevant() && UpdateCounter.HasEverBeenUpdated() && !UpdateCounter.WasSynchronizedCounter(Context.AnimInstanceProxy->GetUpdateCounter());
...
void FCitySampleAnimNode_RequestInertializationOnAssetChange::Update_AnyThread(const FAnimationUpdateContext& Context)
{
	...
	if (AssetPlayerNode != nullptr)
	{
		...
		const bool bSkipForBecomingRelevant = UpdateCounter.HasEverBeenUpdated() && !UpdateCounter.WasSynchronizedCounter(Context.AnimInstanceProxy->GetUpdateCounter());
		UpdateCounter.SynchronizeWith(Context.AnimInstanceProxy->GetUpdateCounter());
	...
	{
		FText Message = LOCTEXT("RequestInertializationOnAssetChangeError", "No asset player node found as the source of FAnimNode_RequestInertializationOnAssetChange.");
