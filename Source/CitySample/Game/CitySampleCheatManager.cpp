...
void UCitySampleCheatManager::TuneCurrentVehicle()
{
	if (ACitySamplePlayerController* const MyPlayerController = Cast<ACitySamplePlayerController>(GetOuterAPlayerController()))
	{
		...
		{
			MyPlayerController->ConsoleCommand(*FString::Printf(TEXT("editobject %s"), *(Vehicle->GetVehicleMovementComponent()->GetPathName())));
			MyPlayerController->ConsoleCommand(*FString::Printf(TEXT("editobject %s"), *(Vehicle->GetPathName())));
		}
	}
}
