...
void ACitySamplePlayerController::TryToInteract(UCitySampleInteractionComponent* Interaction)
{
	...
	if (IsInteracting())
	{
		UE_LOG(LogCitySample, Log, TEXT("%s::TryToInteract: Already interacting with %s"), *GetPathName(this), *GetPathName(CurrentInteractionComponent));
		return;
	}

	if (!Interaction)
	{
		UE_LOG(LogCitySample, Log, TEXT("%s::TryToInteract: Invalid interaction"), *GetPathName(this));
		return;
	}

	if (!Interaction->TryToLockInteraction(this))
	{
		UE_LOG(LogCitySample, Log, TEXT("%s::TryToInteract: Interaction is busy %s"), *GetPathName(this), *GetPathName(Interaction));
		return;
	}
...
void ACitySamplePlayerController::TryToUpdateInteraction(UCitySampleInteractionComponent* ToInteractWith, uint8 InInteractionState)
{
	...
	if (!ToInteractWith)
	{
		UE_LOG(LogCitySample, Error, TEXT("TryToUpdateInteraction_Validate:%s: Invalid Interaction Object. Object=%s, Type=%s"), *GetPathName(this), *GetPathNameSafe(ToInteractWith), *InteractionStateToText(InInteractionState).ToString());
		return;
	}

	if (!IsInteractionStateValid(InInteractionState))
	{
		UE_LOG(LogCitySample, Error, TEXT("TryToUpdateInteraction_Validate:%s: Invalid Interaction Type. Object=%s, Type=%s"), *GetPathName(this), *GetPathName(ToInteractWith), *InteractionStateToText(InInteractionState).ToString())
		return;
	}
	...
	auto LogInteraction = [&]()
	{
		UE_LOG(LogCitySample, Log, TEXT("TryToUpdateInteraction:%s: Interaction. Object=%s, Type=%s, Reason=%s"), *GetPathName(this), *GetPathName(ToInteractWith), *InteractionStateToText(InteractionState).ToString(), *InteractionResultToText(InteractionResult).ToString());
	};

	if (InteractionQueue.Num() > 0)
	{
		const FInteractionItem& CurrentlyProcessing = InteractionQueue[0];
		if (!ensure(EInteractionState::Start == static_cast<EInteractionState>(CurrentlyProcessing.InteractionState)) || !ensure(ToInteractWith == CurrentlyProcessing.ToInteractWith))
		{
			// We're trying to start a new interaction while processing an existing one. This should not be possible out of the box
	...
	{
		// Currently, the only ways in which we'll fail a Finish or Abort would be if we aren't interacting with the specified component, 
        // or if the specified component thinks it isn't being interacted with by this character.
	...
	{
		// Push our current interaction onto the stack. That way if we trigger a subsequent one while processing, it knows to deffer itself.
