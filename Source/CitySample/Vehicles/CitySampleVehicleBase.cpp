...
void ACitySampleVehicleBase::BeginPlay()
{
	...
	// Because we don't need an AI controller to move. MassTraffic will take care of that in the AI case and the player controller in the player case.
...
void ACitySampleVehicleBase::PossessedBy(AController* NewController)
{
	...
	if (CitySamplePlayerController != nullptr && CitySamplePlayerController == NewController)
	{
		...
		const UWorld* World = GetWorld();
		{
			...
			{
				// An entity might be removed from Mass (because the vehicle was damaged and LODs out), hence the thorough IsEntityValid() check.
...
void ACitySampleVehicleBase::UpdateVehicleDriftingState()
{
	if (const UChaosWheeledVehicleMovementComponent* const MovementComponent = Cast<UChaosWheeledVehicleMovementComponent>(GetVehicleMovementComponent()))
	{
		...
		bIsDrifting =  ((GetWheelSlipAngleInDegrees(2) >= RearWheelSlipAngleDriftingMinThreshold) || (GetWheelSlipAngleInDegrees(3) >= RearWheelSlipAngleDriftingMinThreshold)) && (FMath::Abs(DrivingState.ForwardSpeed) >= DriftingSpeedMinThreshold);
